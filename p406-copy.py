...
class ModernMongoApp:
    ...
    def show_signup_form(self):
        ...
        # Password field
        ctk.CTkLabel(fields_frame, text="Password:", font=("Arial", 16, "bold"), text_color="white").pack(anchor="w",
                                                                                                          pady=(0, 5))
    ...
    def show_products(self):
        ...
        # 🔑 Ensure we always keep image references alive
        ...
        if self.products_list:
            for p in self.products_list:
                ...
                row.pack(fill="x", pady=15, padx=30)

                # Product Image (with rounded corners)
                img_url = p.get("image_url")
                if img_url:
                    try:
                        # Check if URL is web or local path
                        if img_url.startswith("http://") or img_url.startswith("https://"):
                            response = requests.get(img_url, timeout=5)
                            pil_img = Image.open(io.BytesIO(response.content)).convert("RGBA")
                        else:
                            pil_img = Image.open(img_url).convert("RGBA")  # local file

                        pil_img = pil_img.resize((100, 100))
                        pil_img = round_corners(pil_img, radius=5)
                        ...
                        print("Image load error:", e, img_url)
